                      ...
                      {showDev && (
                        <tr style={{background:Math.abs(pct)>=20?'#fef2f2':'#fffbeb'}}>
                          <td colSpan={10} style={{padding:'6px 10px 8px 32px'}}>
                            <div style={{display:'flex',alignItems:'center',gap:8}}>
                              <span style={{fontSize:11,fontWeight:700,color:Math.abs(pct)>=20?'#dc2626':'#d97706',whiteSpace:'nowrap'}}>{Math.abs(pct)>=20?'⛔':'⚠️'} Deviation reason:</span>
                              {inspInp(r.deviationReason, v=>updLS(r.id,'deviationReason',v), 'Explain deviation from budgeted head count…', 'text', {border:`1px solid ${Math.abs(pct)>=20?'#fca5a5':'#fcd34d'}`,background:'white'})}
                            </div>
                          </td>
                        </tr>
                      )}
